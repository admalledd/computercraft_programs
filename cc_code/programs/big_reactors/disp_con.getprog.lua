...
while true do
    ...
    monitor.write('Control Rod Depth:  '..(reactor.getControlRodLevel(0))..'%')
    ...
    monitor.setTextColor(colors.white)
    monitor.write('Flux:') --..(0+(math.floor(reactor.getEnergyStored()/100000)))..'%')
    ...
    if rs.getInput('left')==false and emptyflag==0 then
        reactor.setActive(true)
        offlineflag=0
    end
    
    if rs.getInput('left')==true and emptyflag==0 then
        reactor.setActive(false)
        reactor.setAllControlRodLevels(100)
        offlineflag=1
    end    
